...
Vagrant::Config.run do |config|
  ...
  config.vm.host_name = "scafandru"

  config.vm.box = "precise64bootstrap"
  config.vm.box_url = "http://dl.dropbox.com/u/3914655/precise64bootstrap.box"
  ...
  config.vm.provision :chef_solo do |chef|
    ...
    chef.json = {
      :mysql => {
        ...
        :database_name            => 'scafandru_dev',
        :database_user_name       => 'scafandru_app',
        :database_user_password   => 'supersecret'
      }
    }

    chef.run_list = [
      "recipe[scafandru::database]",
      "recipe[scafandru::default]"
    ]
  end
end
